...
from prompt_template import generate_prompt_templates
...
__all__ = ["available_models", "load", "tokenize" ,"encode_text_with_disc_prompt_ensemble",              "encode_text_with_corr_prompt_ensemble"]
...
def encode_text_with_disc_prompt_ensemble(model, texts, device, prompt_templates=None):
...
def encode_text_with_corr_prompt_ensemble(model, cls_texts, coo_cls_texts, device, prompt_templates=None):

    # using default prompt templates for ImageNet
    
    text_features = []
    for t in cls_texts:
        prompt_templates = generate_prompt_templates(len(coo_cls_texts[t]))
        cls_ = [t] + coo_cls_texts[t]
        prompted_t = [template.format(*cls_) for template in prompt_templates]
        prompted_t = tokenize(prompted_t).to(device)
        class_embeddings = model.encode_text(prompted_t)
        class_embeddings /= class_embeddings.norm(dim=-1, keepdim=True)
        class_embedding = class_embeddings.mean(dim=0)
